...
let newSellOrders = [];
let pair = "BTF-AVAX";
let pairBuy = "BTF-AVAX-BUY";
let pairSell = "BTF-AVAX-SELL";
let pairTrade = "BTF-AVAX-TRADES"
...
function processTrade(trade){
    ...
    let tokenGetAddress = taker.side === 'bid' ? AVAX : BTF;
    let tokenGiveAddress = taker.side === 'bid' ? BTF : AVAX;
...
const sellOrdersSocket = () => {
    const q = query(collection(db, `BTF-AVAX-SELL`), where("pair", "==", "BTF-AVAX"));
...
const buyOrdersSocket = () => {
    const q = query(collection(db, `BTF-AVAX-BUY`), where("pair", "==", "BTF-AVAX"));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
        const orders = [];
        querySnapshot.forEach((doc) => {
            orders.push(doc.data());
